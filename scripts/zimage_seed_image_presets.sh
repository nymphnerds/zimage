...
PACKAGED_PRESET_DIR="${PACKAGED_PRESET_DIR:-$MODULE_ROOT/prompt_presets}"
MARKER_FILE="${TARGET_DIR}/.defaults_seeded"
FORCE_IF_EMPTY="${ZIMAGE_SEED_IMAGE_PRESETS_IF_EMPTY:-0}"

mkdir -p "${TARGET_DIR}"

if [[ -f "${MARKER_FILE}" && "${FORCE_IF_EMPTY}" != "1" ]]; then
  echo "directory=${TARGET_DIR}"
  exit 0
fi

if [[ -f "${MARKER_FILE}" && "${FORCE_IF_EMPTY}" == "1" ]] && find "${TARGET_DIR}" -maxdepth 1 -type f -name '*.json' | grep -q .; then
  echo "directory=${TARGET_DIR}"
  exit 0
fi

if [[ ! -d "${PACKAGED_PRESET_DIR}" ]]; then
  echo "preset_source_missing=${PACKAGED_PRESET_DIR}" >&2
  echo "directory=${TARGET_DIR}"
  exit 0
fi

TARGET_DIR="${TARGET_DIR}" PACKAGED_PRESET_DIR="${PACKAGED_PRESET_DIR}" python3 - <<'PY'
import json
import os
import re
